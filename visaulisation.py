...
def visualize_multi_class():
    df = pd.read_csv("Results_for_visualization_multi")
...
def visualize_binary_class():
    df = pd.read_csv(os.path.join(os.getcwd(), 'Data/Results_for_visualization_binary.csv'), delimiter=',')
    new_palette = sns.color_palette("husl", n_colors=len(df.columns))
    ...
    plt.title("Results of Evaluation Measures over BCCA")
